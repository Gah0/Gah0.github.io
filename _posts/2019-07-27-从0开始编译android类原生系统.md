...
> 胆子大去学校机房找台性能好的电脑编译，安装虚拟机，特别是做视频剪辑的那种，我学校配置一般都是i7的，系统编译一个小时就出来了
...
> 如果显示libxml2-utils_2.9.4-1+deb.sury.org~xenial+2_amd64.deb 找不到，无法直接安装。
...
> 将 https://mirrors.tuna.tsinghua.edu.cn/git/git-repo 
>
> 替换为 [git://mirrors.ustc.edu.cn/aosp/platform/manifest](git://mirrors.ustc.edu.cn/aosp/platform/manifest)   
>
> 就是中科大的源啦
...
```
 ...
 - **设置git身份，添加自己的邮箱和姓名：**
     ...
     ```
     git config --global user.email "woshishacha3@gmail.com"
...
源码下载完成后，看到下面的信息可以说明下载成功。整个源码的大小为45+g
...
> 大家可以参考我这个源的提交(往后会更新)[android_xiaomi_kernel_grus][7]
...
- **别人的做法**
  ...
  我看过一个开发者的做法，我感觉很妥当，能保持内核干净，减少底层无用代码，把caf的tag先克隆下来，然后再cherry-pick小米的提交，把该手机驱动无关的部分剔除，将有关驱动逐个合并到caf tag上。类似于[我对小米4c的变基操作][21]。然后再盖上当前linux-stable最新版本3.10.108...其他手机同理！
