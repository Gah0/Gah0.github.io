>EAS是一种先进的，能源管理调度，它能根据节点数据和当前CPU负载，对arm处理器处理器进行升频，降频的操作。这将使得ARM芯片更加节省电能（HMP因为回调操作过慢，不符合能源效率的原因，被抛弃了。）
...
上面的循环，当if条件sysctl_sched_is_big_little成立时，将提示基于cpu选择，而不是基于集群选择。（感觉基于集群选择才是正确，奈何手机性能不满足）
